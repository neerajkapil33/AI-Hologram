...
function App() {
  ...
  const [response, setResponse] = useState('Ask Neeraj about career growth, leadership, interviews, skills or opportunities.');
  ...
  const startListening = () => { const speechWindow = window as SpeechWindow; const Recognition = speechWindow.SpeechRecognition ?? speechWindow.webkitSpeechRecognition; if (!Recognition) { setStatus('VOICE INPUT NOT SUPPORTED • USE THE TEXT BOX'); return; } const recognition = new Recognition(); recognition.lang = language; recognition.continuous = false; recognition.interimResults = false; recognition.onresult = (event) => processQuestion(event.results[0]?.[0]?.transcript ?? ''); recognition.onerror = () => { setListening(false); setStatus('VOICE INPUT ERROR • TRY AGAIN'); }; recognition.onend = () => setListening(false); recognitionRef.current = recognition; setListening(true); setMode('companion'); setStatus(`LISTENING • ${language}`); command({ type: 'expression', value: 'neutral' }); recognition.start(); };
  const toggleVoice = () => { if (listening) { recognitionRef.current?.stop(); setListening(false); return; } startListening(); };
  const startVideoCall = async () => { setMode('companion'); setStartingCall(true); setStatus('CONNECTING • HIGH-FIDELITY NEERAJ AI REPLICA'); try { const res = await fetch(`${API_BASE_URL}/api/tavus/conversation`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ language }) }); if (!res.ok) throw new Error(`LIVE API ${res.status}`); const room = await res.json() as LiveRoom; setLiveRoom(room); if (room.conversation_url) setStatus('LIVE VIDEO CALL • NEERAJ AI IS HERE'); else setStatus(room.error ?? room.message ?? 'LIVE CALL IS NOT CONFIGURED YET'); } catch (error) { setStatus(`VIDEO CALL ERROR • ${error instanceof Error ? error.message : 'TRY AGAIN'}`); } finally { setStartingCall(false); } };
  return <main className="neeraj-screen">
    <style>{`.header-center{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:min(58vw,680px);text-align:center;pointer-events:none;display:flex;flex-direction:column;align-items:center;justify-content:center;line-height:1}.header-neeraj{font-size:24px;font-weight:900;letter-spacing:9px;padding-left:9px;color:#f2fcff;text-shadow:0 0 18px rgba(81,221,255,.28)}.header-sub{margin-top:7px;font-size:8px;font-weight:700;letter-spacing:2.2px;color:#51ddff;text-transform:uppercase}.header-status{display:flex;align-items:center;gap:7px;margin-top:8px;font-size:7px;font-weight:800;letter-spacing:1.4px;color:#20ff92;text-transform:uppercase}.header-status i{display:block;width:6px;height:6px;border-radius:50%;background:#20ff92;box-shadow:0 0 12px #20ff92}.header-prompt{margin-top:8px;max-width:100%;font-size:8px;line-height:1.25;letter-spacing:.35px;color:#9ebbc6}.neeraj-header .system-pill{flex-shrink:0}@media(max-width:820px){.header-center{position:relative;left:auto;top:auto;transform:none;width:auto;flex:1;min-width:0;margin:0 8px}.header-neeraj{font-size:18px;letter-spacing:6px;padding-left:6px}.header-sub{font-size:5.5px;letter-spacing:1px;margin-top:5px}.header-status{font-size:5.5px;letter-spacing:.7px;margin-top:5px}.header-prompt{display:none}.neeraj-header .brand-lockup{flex-shrink:0}}`}</style>
    <canvas ref={ref} className="hologram-canvas" /><div className="screen-grid" />
    <header className="neeraj-header"><div className="brand-lockup"><div className="brand-orb">N</div><div><div className="brand-name">NEERAJ <span>AI</span></div><div className="brand-line">Career. Growth. Global.</div></div></div><div className="header-center"><div className="header-neeraj">NEERAJ</div><div className="header-sub">REAL-TIME 3D AVATAR • CAREER INTELLIGENCE</div><div className="header-status"><i /> ONLINE • REAL NEERAJ 3D READY</div><div className="header-prompt">Ask Neeraj about career growth, leadership, interviews, skills or opportunities</div></div><div className="system-pill"><i /> ONLINE <b>│</b> WEBGPU <b>│</b> TYPEGPU <b>│</b> REACT</div></header>
    <section className="hero-grid">
      ...
      <aside className="right-rail"><div className="map-card"><div className="eyebrow">GLOBAL REACH</div><strong>75+<small>Countries</small></strong><div className="map-lines">✦　◌　✧　◌　✦</div></div><div className="dual-card"><div><div className="eyebrow">FOCUS AREAS</div><p>◈ Technology</p><p>◇ FinTech</p><p>♡ Healthcare</p><p>ϟ Energy & Power</p><p>◉ Telecom</p><p>♙ Startups</p><p>▣ MNCs</p></div><div className="quote">“Success is not just about reaching the destination, but growing in the journey.”<em>— Neeraj</em></div></div><div className="approach"><div className="eyebrow">MY APPROACH</div><div className="approach-grid"><span>♡<small>Empathetic</small></span><span>♧<small>Strategic</small></span><span>▥<small>Data-Driven</small></span><span>♙<small>People First</small></span></div></div></aside>
    </section>
    <section className="control-deck"><div className="mode-switch"><button className="active">3D AVATAR</button><button onClick={() => setMode('profile')}>PROFILE</button></div><div className="chat-input"><span>◌</span><input placeholder="Type to Chat with Neeraj" onKeyDown={(e) => { if (e.key==='Enter') processQuestion(e.currentTarget.value); }} /><button onClick={toggleVoice}>{listening ? 'STOP' : '🎙'}</button></div><select aria-label="Conversation language" value={language} onChange={(e) => setLanguage(e.target.value)}><option value="en-IN">English</option><option value="hi-IN">हिन्दी</option><option value="ta-IN">தமிழ்</option><option value="te-IN">తెలుగు</option><option value="bn-IN">বাংলা</option><option value="mr-IN">मराठी</option></select><button className="call-button" disabled={startingCall} onClick={startVideoCall}>{startingCall ? 'CONNECTING…' : 'VIDEO CALL'}</button>{liveRoom?.conversation_url && <button className="stop-button" onClick={() => { setLiveRoom(null); setStatus('VIDEO CALL ENDED • 3D NEERAJ AI READY'); }}>END CALL</button>}{speaking && <button className="stop-button" onClick={stopSpeaking}>STOP VOICE</button>}</section>
