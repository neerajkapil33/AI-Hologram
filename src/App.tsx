...
function App() {
  ...
  const processQuestion = (text: string) => {
    ...
    setTranscript(clean); setStatus('NEERAJ THINKING • FORMING YOUR RESPONSE'); command({ type: 'expression', value: 'thinking' }); command({ type: 'gesture', value: 'nod' }); sendText(clean, language);
  };
  ...
  const startListening = () => {
    ...
    const recognition = new Recognition(); recognition.lang = language; recognition.continuous = false; recognition.interimResults = false;
    recognition.onresult = (event) => processQuestion(event.results[0]?.[0]?.transcript ?? ''); recognition.onerror = () => { setListening(false); setStatus('VOICE INPUT ERROR • TRY AGAIN'); }; recognition.onend = () => setListening(false); recognitionRef.current = recognition;
    setListening(true); setMode('companion'); setStatus(`LISTENING • ${language}`); command({ type: 'expression', value: 'neutral' }); recognition.start();
  };
  const toggleVoice = () => { if (listening) { recognitionRef.current?.stop(); setListening(false); return; } startListening(); };

  const startVideoCall = async () => {
    setMode('companion'); setStartingCall(true); setStatus('CONNECTING • HIGH-FIDELITY NEERAJ AI REPLICA');
    try {
      ...
      if (!res.ok) throw new Error(`LIVE API ${res.status}`); const room = await res.json() as LiveRoom; setLiveRoom(room);
      if (room.conversation_url) setStatus('LIVE VIDEO CALL • NEERAJ AI IS HERE'); else setStatus(room.error ?? room.message ?? 'LIVE CALL IS NOT CONFIGURED YET');
    } catch (error) { setStatus(`VIDEO CALL ERROR • ${error instanceof Error ? error.message : 'TRY AGAIN'}`); } finally { setStartingCall(false); }
  };
  ...
  return (
    <main className="neeraj-screen">
      ...
      <section className="control-deck"><div className="mode-switch"><button className={mode==='profile'?'active':''} onClick={() => { setMode('profile'); setLiveRoom(null); }}>PROFILE</button><button className={mode==='companion'?'active':''} onClick={() => setMode('companion')}>AI CAREER COMPANION</button></div><div className="chat-input"><span>◌</span><input placeholder="Type to Chat with Neeraj" onKeyDown={(e) => { if (e.key==='Enter') processQuestion(e.currentTarget.value); }} /><button onClick={toggleVoice}>{listening ? 'STOP' : '🎙'}</button></div><select aria-label="Conversation language" value={language} onChange={(e) => setLanguage(e.target.value)}><option value="en-IN">English</option><option value="hi-IN">हिन्दी</option><option value="ta-IN">தமிழ்</option><option value="te-IN">తెలుగు</option><option value="bn-IN">বাংলা</option><option value="mr-IN">मराठी</option></select><button className="call-button" disabled={startingCall} onClick={startVideoCall}>{startingCall ? 'CONNECTING…' : 'VIDEO CALL'}</button>{liveRoom?.conversation_url && <button className="stop-button" onClick={() => { setLiveRoom(null); setStatus('VIDEO CALL ENDED • NEERAJ AI READY'); }}>END CALL</button>}{speaking && <button className="stop-button" onClick={stopSpeaking}>STOP VOICE</button>}</section>
      <footer className="neeraj-footer"><span>AI CAREER INTELLIGENCE</span><span>VOICE • FACE • EXPRESSION • BODY • BRAIN • MULTILINGUAL</span><span>HOLOGRAM SYSTEM v3.1</span></footer><div className="ai-disclosure">AI representation of Neeraj Kapil • generated responses are not statements made by the physical Neeraj.</div>
    </main>
  );
}
export default App;
