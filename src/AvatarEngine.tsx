...
export default function AvatarEngine({ onStatus, onApi }: Props) {
  ...
  useEffect(() => {
    ...
    loader.load(AVATAR_SOURCE, (gltf) => {
      ...
      statusRef.current?.(`ONLINE • REAL NEERAJ 3D READY${gltf.animations?.length ? ` • ${gltf.animations.length} ANIMATION${gltf.animations.length > 1 ? 'S' : ''}` : ''}${mouthTargets.length || jawTargets.length ? ' • FACIAL VISEMES READY' : ''} • ORGANIC BLINK + GAZE READY`);
