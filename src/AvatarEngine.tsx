...
const loadAvatarOnce = (url: string) => {
  const existing = sharedLoads.get(url); if (existing) return existing;
  const promise = new Promise<LoadedAvatar>((resolve, reject) => new GLTFLoader().load(url, (gltf) => resolve({ scene: gltf.scene, animations: gltf.animations ?? [] }), undefined, reject));
  sharedLoads.set(url, promise); return promise;
};
...
export default function AvatarEngine({ onStatus, onApi }: Props) {
  ...
  useEffect(() => {
    ...
    const prepareModel = (loaded: LoadedAvatar, source: string) => {
      ...
      console.info('[AVATAR DIAGNOSTIC] Bounds:', new THREE.Box3().setFromObject(model).getSize(new THREE.Vector3()), 'mouthTargets:', mouthTargets.length, 'jawTargets:', jawTargets.length);
    };
    ...
    return () => { disposed = true; renderer.setAnimationLoop(null); observer.disconnect(); microExpressions?.dispose(); mixer?.stopAllAction(); resetTargets(mouthTargets); resetTargets(jawTargets); if (model) avatarRoot.remove(model); renderer.dispose(); floor.geometry.dispose(); (floor.material as THREE.Material).dispose(); grid.geometry.dispose(); gridMaterials.forEach((m) => m.dispose()); innerRing.geometry.dispose(); (innerRing.material as THREE.Material).dispose(); outerRing.geometry.dispose(); (outerRing.material as THREE.Material).dispose(); canvasContainer.remove(); };
  }, []);
