...
export default function AvatarEngine({ onStatus, onApi }: Props) {
  ...
  useEffect(() => {
    ...
    const masterAmbient = new THREE.AmbientLight(0x0f2042, 1.8); scene.add(masterAmbient);
    const keyCyanLight = new THREE.DirectionalLight(0x8fe9ff, 2.45); keyCyanLight.position.set(3, 5, 3); scene.add(keyCyanLight);
    const secondaryMagentaLight = new THREE.DirectionalLight(0xd946ef, 0.45); secondaryMagentaLight.position.set(-3, 3, -2); scene.add(secondaryMagentaLight);
    const neutralFill = new THREE.DirectionalLight(0xffffff, 1.1); neutralFill.position.set(-2, 2.5, 4); scene.add(neutralFill);
    ...
    const grid = new THREE.GridHelper(6, 24, 0x06b6d4, 0x111e36); grid.position.y = 0.015;
    const gridMaterials = Array.isArray(grid.material) ? grid.material : [grid.material];
    gridMaterials.forEach((material) => { material.transparent = true; material.opacity = 0.12; }); stage.add(grid);
    ...
    innerRing.rotation.x = -Math.PI / 2; innerRing.position.y = 0.025; stage.add(innerRing);
    ...
    outerRing.rotation.x = -Math.PI / 2; outerRing.position.y = 0.028; stage.add(outerRing);
    ...
    const frameModel = (root: THREE.Object3D) => {
      ...
      const rawHeight = Math.max(rawSize.y, 0.001); const scale = Math.min(1.48 / rawHeight, 1);
      ...
      const height = Math.max(size.y, 0.001); root.position.set(-center.x, -box.min.y, -center.z); root.updateMatrixWorld(true);
    ...
    renderer.setAnimationLoop(() => {
      ...
      mixer?.update(dt); microExpressions?.update(performance.now());
      if (mouthTargets.length || jawTargets.length) { const target = speaking ? Math.max(0.04, intensity * 0.5) : 0; setTargets(mouthTargets, target); setTargets(jawTargets, Math.min(target * 0.5, 0.45)); }
