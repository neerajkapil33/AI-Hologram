...
export default function AvatarEngine({ onStatus, onApi }: Props) {
  ...
  useEffect(() => {
    ...
    scene.add(new THREE.HemisphereLight(0xd7e8ef, 0x07101a, 1.5));
    const key = new THREE.DirectionalLight(0xffffff, 2.0);
    key.position.set(2.5, 4.5, 4);
    scene.add(key);
    const rim = new THREE.DirectionalLight(0x74ddff, 1.2);
    rim.position.set(-3, 2.5, -3);
    scene.add(rim);
    const fill = new THREE.PointLight(0x4a8dff, 0.7, 6);
    fill.position.set(0, 1.8, 1.5);
    scene.add(fill);
    ...
    const floor = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x071019, metalness: 0.4, roughness: 0.48 }),
    );
    ...
    const floorRing = new THREE.Mesh(
      new THREE.RingGeometry(0.82, 0.84, 128),
      new THREE.MeshBasicMaterial({ color: 0x8bd6e4, transparent: true, opacity: 0.5, side: THREE.DoubleSide }),
    );
    ...
    const stageRing = new THREE.Mesh(
      new THREE.TorusGeometry(0.86, 0.009, 8, 128),
      new THREE.MeshBasicMaterial({ color: 0xa5e0e8, transparent: true, opacity: 0.36, side: THREE.DoubleSide }),
    );
    stageRing.rotation.x = Math.PI / 2;
    stageRing.position.y = 0.045;
    stage.add(stageRing);
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const r = 0.82 + Math.random() * 0.58;
      positions[i * 3] = Math.cos(a) * r;
      positions[i * 3 + 1] = 0.08 + Math.random() * 2.5;
    ...
    const particles = new THREE.Points(
      pg,
      new THREE.PointsMaterial({ color: 0xc5e8ee, size: 0.008, transparent: true, opacity: 0.28, depthWrite: false }),
    );
    stage.add(particles);
    ...
    const findMouthTarget = (root: THREE.Object3D) => {
      ...
      root.traverse((object) => {
        ...
        const name = names.find((n) => /mouth.?open|jaw.?open|viseme.?aa|viseme.?sil/i.test(n));
        if (name) best = { mesh: object, index: object.morphTargetDictionary[name] };
      });
      return best;
    };
    ...
    const command = (cmd: AvatarCommand) => {
      ...
      if (cmd.type === 'viseme' && mouthTarget) {
        const value = Math.max(0, Math.min(1, Number(cmd.weight ?? 0)));
